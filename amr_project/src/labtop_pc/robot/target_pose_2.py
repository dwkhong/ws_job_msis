...
def build_target_pose(current_tcp_pose6: Pose6, measure_res: MeasureDict) -> List[float]:
    ...
    # [B] RY 보정: 박스 기울기에 맞춰 그리퍼 각도 변경 + Pivot 위치 보정
    # L(PIVOT_LENGTH): J4 축에서 TCP 끝점까지의 거리
    L = float(getattr(rc, "PIVOT_LENGTH", 165.0)) 
    
    # ✅ [변경 핵심] 2.0(고정) 대신 비전 측정값 사용
    # 값이 없으면(이전 버전 비전 코드 등) 안전하게 2.0 사용
    measured_ry = float(measure_res.get("ry_deg", 2.0))
    
    # 너무 과도한 각도로 꺾이지 않게 안전장치 (선택사항, 필요 없으면 제거 가능)
    # 예: -45도 ~ 45도 사이로 제한
    # if measured_ry > 45.0: measured_ry = 45.0
    # if measured_ry < -45.0: measured_ry = -45.0

    target_ry = measured_ry
    
    rad_curr = np.deg2rad(ry)
    rad_targ = np.deg2rad(target_ry)

    # 팔이 기울어질 때(target_ry) 발생하는 로컬 변위 계산 (삼각함수 차이)
    # Pivot 위치가 변하기 때문에 XYZ를 역보정해줘야 끝점이 박스에 닿음
    ...
    # [C] RZ 각도 보정: 카메라가 측정한 박스 회전 각도 반영
...
def build_target_pose_with_debug(current_tcp_pose6: Pose6, measure_res: MeasureDict) -> Dict[str, object]:
    ...
    return {
        ...
        "move_z_mm": move_z,
        "current_ry": ry,
        "target_ry": target[4], # 이제 이 값이 측정된 ry_deg가 됩니다.
        "angle_deg": float(measure_res.get("angle_deg", 0.0)),
        "vision_ry": float(measure_res.get("ry_deg", 0.0)) # 디버깅용 원본 데이터
    }
