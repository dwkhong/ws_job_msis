...
# ============================================================
# ✅ NEW: GRIPPER CONFIG (10번/9번 후 자동닫기)
# ============================================================
GRIPPER_INDEX = 1
...
# ✅ NEW: 11번 스택 사이클 설정
# ============================================================
# 11번이 "먼저 거쳐가는" 위치 (Waypoint A)
WP11_A_POSE = [76.752, 218.531, 466.100, -176.747, 11.609, -128.123]
WP11_A_JOINT = [-134.487, -111.775, 75.336, -65.453, -91.956, 83.763]

# 11번 "도착" 기본 위치 (Drop base)
WP11_DROP_BASE_POSE = [175.145, 312.801, 228.808, -177.820, 1.463, -127.365]

# 누를 때마다 Z를 얼마나 올릴지 (카운터로 조절)
STACK_Z_STEP_MM = 48.0

# 안전상 Z 상한(필요시 조절/삭제)
STACK_Z_MAX_MM = 600.0  # 예: 600mm 넘기면 stop (원하면 None 처리 가능)
...
def main():
    ...
    # ✅ "맨처음 저장한 1번 포지션" (처음 한 번만 세팅)
    home_joint6 = None

    # ✅ NEW: gripper state + counter
    state = {
        "gripper_activated": False,
        "gripper_closed": None,   # True/False/None
        "stack_counter": 0,       # ✅ 11번에서 사용
    }
    ...
    try:
        while True:
            ...
            if cmd == "1":
                ...
                # ✅ 홈은 "맨처음 1번 눌렀을 때"만 저장 (이후 1번 눌러도 유지)
                if home_joint6 is None:
                    home_joint6 = joint6[:]
