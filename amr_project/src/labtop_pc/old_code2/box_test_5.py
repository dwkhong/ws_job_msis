...
# -----------------------------
# ✅ Depth ROI 안정화 파라미터 (depth는 m 단위)
# -----------------------------
ROI_MARGIN_PX  = 6
MIN_ROI_PIXELS = 120
MAD_THRES_M    = 0.020
...
def main():
    ...
    print(f"[INFO] BASE_YAW_OFFSET_DEG = {BASE_YAW_OFFSET_DEG:+.1f} deg")
    print(f"[INFO] FLIP_MOVE_X={FLIP_MOVE_X}, FLIP_MOVE_Y={FLIP_MOVE_Y}")
    ...
    try:
        while True:
            ...
            # ============================================================
            # ✅ 핵심 변경: 여러 박스가 잡히면 "이미지 중심에 가장 가까운 것" 선택
            #   우선순위: (중심거리 최소) -> (conf 최대)
            # ============================================================
            best = None  # (dist2_to_img_center, -conf, conf, cls, poly4x2)
            ...
            if getattr(r, "obb", None) is not None and r.obb is not None:
                ...
                if obb.xyxyxyxy is not None and len(obb.xyxyxyxy) > 0:
                    ...
                    for poly8, cf, ci in zip(polys[keep], confs[keep], clss[keep]):
                        ...
                        cand = (dist2, -float(cf), float(cf), int(ci), poly)
            ...
            print(
                f"[{len(accepted)}/{AVG_N}] conf={cur['conf']:.2f} "
        ...
        cam_arr = np.array(
            [
                [a["Xmm"], a["Ymm"], a["Zmm"], a["distmm"], a["angle"], a["Zdepth_mm"], a["Zsize_mm"], a["Zuse_mm"]]
                for a in accepted
            ],
            dtype=np.float32,
        )
        ...
        print(f"\n[CAMERA] (mm)")
        print(
            f"XYZ avg (mm)  : ({cam_mean[0]:+.1f}, {cam_mean[1]:+.1f}, {cam_mean[2]:+.1f})   "
        ...
        print(
            f"XYZ avg (mm)  : ({g_mean[0]:+.1f}, {g_mean[1]:+.1f}, {g_mean[2]:+.1f})   "
