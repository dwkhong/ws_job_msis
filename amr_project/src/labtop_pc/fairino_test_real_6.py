...
# ✅ 8번(초기 위치 복귀) config
# -------------------------
MOVEJ_VEL_RETURN    = 15.0  # 초기 자세로 돌아갈 때 속도(%)
...
def prompt_menu(step_scale):
    ...
    print("  q : 종료")
    print("=======================================")
    return input("입력 (1/2/3/4/5/6/7/8/q) > ").strip()
...
def main():
    ...
    reached_final = False    # ✅ 최종 도달하면 True (도달 후 7번 재실행 방지)

    # ✅ 초기(프로그램 시작) 상태 저장
    initial_pose6 = None
    initial_joint6 = None
    try:
        (e1, p0), (e2, j0) = read_pose_joint(robot, reconnect=reconnect)
        if e1 == 0 and e2 == 0:
            initial_pose6 = p0[:]
            initial_joint6 = j0[:]
            print("[INIT] 프로그램 시작 초기 위치 저장 ✅")
            print("  init_pose :", fmt_pose6(initial_pose6))
            print("  init_joint:", fmt_joint(initial_joint6))
            print()
        else:
            print(f"[INIT-WARN] 초기 위치 저장 실패: err_p={e1}, err_j={e2}")
            print("           -> 1번을 누를 때 초기값으로 저장 시도합니다.\n")
    except Exception as e:
        print(f"[INIT-WARN] 초기 위치 저장 예외: {e}")
        print("           -> 1번을 누를 때 초기값으로 저장 시도합니다.\n")
    ...
    try:
        while True:
            ...
            if cmd == "1":
                ...
                # ✅ 초기 저장이 비어있으면 1번 시점 값을 초기로 저장
                if initial_joint6 is None:
                    initial_pose6 = pose6[:]
                    initial_joint6 = joint6[:]
                    print("\n[INIT] 초기 위치가 비어있어서, 지금 값을 초기 위치로 저장 ✅")
                    print("  init_pose :", fmt_pose6(initial_pose6))
                    print("  init_joint:", fmt_joint(initial_joint6))
                ...
                approach_phase = 0
                reached_final = False  # ✅ 새 목표면 최종도달 상태 해제
                ...
                if reached_final:
                    print("\n[INFO] 이미 최종(targetZ) 도달 상태입니다 ✅")
                    print("       - 다시 작업하려면 3번(새 target 생성) 또는 8번(초기 복귀) 후 진행하세요.\n")
                    continue
                                    ...
                                    if a1 == 0 and a2 == 0:
                                        ...
                                        if approach_phase == 0:
                                            ...
                                            if z_done:
                                                reached_final = True
                                                approach_phase = 0
                                                print("\n[DONE] ✅ 최종 targetZ 도달했습니다! (프로그램은 계속 실행)")
                                                print("       - 8번을 누르면 초기 위치로 원복됩니다.")
                                                print("       - 새 작업이면 1/2/3 다시 진행하세요.\n")
                ...
                if rtn == 0:
                    ...
                    (a1, pose_after), (a2, joint_after) = read_pose_joint(robot, reconnect=reconnect)
                    if a1 == 0 and a2 == 0:
                        print("\n[RETURN DONE] ✅ 초기 위치 복귀 완료")
                        print("  pose :", fmt_pose6(pose_after))
                        print("  joint:", fmt_joint(joint_after))
                    else:
                        print(f"\n[RETURN DONE] ✅ 복귀는 시도 완료(상태 읽기 실패 err_p={a1}, err_j={a2})")

                    # ✅ 상태 리셋(다음 작업 대비)
                    approach_phase = 0
                    reached_final = False
                    print("\n[RESET] phase=0, reached_final=False 로 초기화했습니다.\n")
                else:
