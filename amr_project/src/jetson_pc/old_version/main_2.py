...
def main():
    ...
    last_tcp_pose = None
    last_measure = None
    last_phase0 = None  # ✅ [추가] IK가 계산한 안전 진입점 저장용
    ...
    while True:
        ...
        if cmd == "0":
            ...
            out = ika.cmd4_check_and_adjust_target_only(
                ...
                target_pose6=last_target_pose, # ✅ 이제 ika가 이걸 받도록 수정했으므로 OK
            )
            ...
            last_phase0 = out["phase0"]  # ✅ [추가] 계산된 진입점 저장!
            ...
            out = sa.cmd9_smooth_auto(
                ...
                reconnect=reconnect,
                last_target_pose6=last_target_pose,
                safe_phase0_pose=last_phase0,  # ✅ [수정] 저장해둔 phase0 전달
                last_measure=last_measure,
                state=state,
                tool=0,
                user=0,
            ...
            for i in range(loop_cnt):
                ...
                phase0 = ik_out["phase0"] # ✅ [추가] 여기서 phase0을 받아야 함
                ...
                out_pick = sa.cmd9_smooth_auto(
                    ...
                    safe_phase0_pose=phase0,  # ✅ [수정] 방금 구한 phase0 전달
                    last_measure=res,
                    state=state,
                    tool=0, user=0,
                    auto_grip_close=True
