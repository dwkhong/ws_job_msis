...
def rotate_j6_by_delta(
    ...
    기본값(robot_config):
      ...
      - (옵션) delta clamp: rc.J6_MAX_STEP_DEG
    """
    if tool is None:
    ...
    max_step = float(getattr(rc, "J6_MAX_STEP_DEG", 9999.0))
...
def cmd_rotate_j6_prompt(
    ...
    print("\n[ACTION] Rotate J6 by user input delta")
    print(f"  delta(J6) = {delta:+.3f} deg")
...
def cmd11(robot, reconnect=None) -> Dict[str, Any]:
    """
    ✅ main의 11번용: J6 delta 입력받아 회전
    """
    if robot is None:
        print("[11] Robot not connected. (0번 먼저)")
        return {"ok": False, "msg": "robot is None", "err": -1, "delta": 0.0}

    return cmd_rotate_j6_prompt(robot, reconnect=reconnect)
