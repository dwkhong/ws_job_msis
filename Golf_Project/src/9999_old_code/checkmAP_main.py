...
# libPath = os.path.join(currentPath, '_lib')
# add_path(libPath)

# import _init_paths
from _lib.BoundingBox import BoundingBox
from _lib.BoundingBoxes import BoundingBoxes
from _lib.Evaluator import *
from _lib.utils import BBFormat
...
def main(args):
    ...
    for metricsPerClass in detections:
        ...
        Avg_conf_TP = metricsPerClass['Avg Conf Score TP']
        Avg_conf_FP = metricsPerClass['Avg Conf Score FP']
        ...
        if _ap is not None: 
            _ap.append({'AP': ap,   'Precision': precision, 'Recall': recall,
                        ...
                        "Conf_TP":Avg_conf_TP, "Conf_FP":Avg_conf_FP})
    ...
    if args.val and _ap is not None:
        s = ('%20s' + '%11s' * 8) % ('Class', 'TP', 'FP', 'P', 'R', 'mAP@.5', "FP %", "TP Conf", "FP Conf")
        print(f'{Style.BRIGHT}{s}')
        mAP_str = ('%20s' + '%11s' * 6) % ('ALL', TP, FP, round(prec*100, 2), round(rec*100, 2), round(mAP*100,2), round(FP_percent*100, 2))
        print(f'{mAP_str}')
        for i in range(len(_ap)):
            ...
            TP_i        =  class_i['TP']
            FP_i        =  class_i['FP']
            Conf_TP_i   =  class_i["Conf_TP"] 
            Conf_FP_i   =  class_i["Conf_FP"] 
            FN_i        =  Positives_i - TP_i
            prec_i      =  TP_i / (TP_i + FP_i)
            rec_i       =  TP_i / (TP_i + FN_i)
            FP_percent_i = FP_i / (TP_i+FP_i)
            mAP_str_i   =  ('%20s' + '%11s' * 8) % (args.names[i], 
                                                    round(TP_i,2), 
                                                    round(FP_i, 2), 
                                                    round(prec_i*100, 2), 
                                                    round(rec_i*100, 2), 
                                                    round(class_i['AP']*100,2),
                                                    round(FP_percent_i*100,2),
                                                    round(Conf_TP_i, 3),
                                                    round(Conf_FP_i, 3),
                                                    )
            print(f'{mAP_str_i}')
